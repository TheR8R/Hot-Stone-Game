  ...
  public CardFigure(Card associatedCard, Point position) {
    super(HotStoneFigureType.CARD_FIGURE,
            ...
            Map.ofEntries(
                    ...
                    entry(FigureType.EFFECT_DESCRIPTION, new Point(48, 170))));
    ...
    writeLock().lock();
    try {
      Point effectD = (Point) position.clone();
      effectD.translate(positions.get(FigureType.EFFECT_DESCRIPTION).x,
              positions.get(FigureType.EFFECT_DESCRIPTION).y);
      if(associatedCard.getEffectDescription() != null) {
        effectText = new TextFigure(associatedCard.getEffectDescription(),
                effectD, Color.WHITE, 16);
      } else {
