...
public class HotStoneDrawing implements Drawing, GameObserver {
  ...
  public void onCardPlay(Player who, Card card) {
    ...
    if(card.getEffectDescription() != null) {
      addMessage("" + card.getName() + " has effect: " + card.getEffectDescription());
    }
  ...
  public void onTurnChangeTo(Player playerBecomingActive) {
    ...
    refreshField(playerBecomingActive);
  ...
  public void onAttackHero(Player playerAttacking, Card attackingCard) {
    // TODO: Inform player
    addMessage("" + playerAttacking + " attacks " + Utility.computeOpponent(playerAttacking)
                  + " with " + attackingCard.getName());
  }
  ...
  public void onUsePower(Player who) {
    // TODO: Inform player
    addMessage(who + " uses their hero power " + game.getHero(who).getEffectDescription());
  }
